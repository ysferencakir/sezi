...
async def create_tables() -> None:
    """Sadece hiç var olmayan tabloları oluşturur (create_all mevcut tabloya sütun eklemez) —
    boş/yeni bir veritabanında ilk açılış için kolaylık. Var olan tablolara sütun eklemek veya
    şema değiştirmek artık Alembic üzerinden yapılır: `alembic revision --autogenerate` + review
    + `alembic upgrade head` (bkz. alembic/README, README.md kurulum adımları). Önceden burada
    HealthDay için sütunları elle ALTER TABLE ile tamamlayan bir hack vardı — Alembic zinciri
    kurulduktan sonra (2026-07-20) kaldırıldı, aynı işi artık migration'lar yapıyor."""
